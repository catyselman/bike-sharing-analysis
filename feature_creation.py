...
def categorize_time(r):
    ...
    """
    response = (
        (r["hr"] >= 5 and r["hr"] < 10) or (r["hr"] >= 16 and r["hr"] < 20)
    ) and r["workingday"]
    return response
...
def weather_cluster(data):
    """
    Creates a column that gives a cluster id based on KMeans clustering of only
...
def deep_features(data):
    """
    Performs deep feature synthesis on the dataframe to generate new columns
    ...
    es = es.entity_from_dataframe(
        entity_id="bikeshare_hourly", index="instant", dataframe=df
    )

    es = es.normalize_entity(
        base_entity_id="bikeshare_hourly",
    ...
    f_mtx, f_defs = ft.dfs(
        ...
        agg_primitives=["std", "max", "min", "mean"],
    )
...
def subcount_forecast(data, feature):
    ...
    :param data: a Dask dataframe where each row is an hour
    :param feature: a String containing the feature that should be forecasted
    (one of: casual, registered)
    :return: a Dask dataframe containing the new column
    """
    ...
    print(
        "\t\tPredictions for GridSearchCV on {}: {:.5f} +/- {:.5f}".format(
            feature,
    ...
    preds = grid_search.best_estimator_.predict(tempDF.drop("cnt", axis=1))

    predicted = dd.from_pandas(
        pd.Series(preds),
        npartitions=5,
    )
...
def prediction_forecasts(data):
    """
    Creates two new columns, one that is predictions for the 'casual' column
    and the other for the 'registered' column

    We wouldn't actually have the registered or casual variables at the time of
    predicting the "cnt" variable, but we
    do have all the other information and we have past information for
    registered and count. So the theory here is that
    we can use predictions for these values that would be accurate enough
    to be extremely helpful as features for a model

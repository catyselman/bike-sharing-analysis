...
def tree_selection(data):
    """
    Uses the feature importances of a decision tree to select the most relevant
    features (importance > 0.1%)
    ...
    """
    print(
        "\tPerforming Tree selection, removing features with" +
        " importance less than 0.1%"
    )
    ...
    for i, u in enumerate(importances):
        if u < 0.001:
            print(
                "\t\tDropping {0}, importance of only {1:.5f}%".format(
                    X.columns[i], u * 100
                )
            )
...
def r2_selection(data):
    """
    Perform forward stepwise feature selection to try and maximize R^2 with the
    least features possible

    :param data: a pandas dataframe where each row is an hour
    :return: a pandas dataframe with only those columns that increased the R^2
    ...
    print(
        "\tPerforming forward stepwise feature selection based on a {0} and R2"
    ...
    while True:
        ...
        if to_check[best_index] > best_r2:
            ...
            print(
                "\t\tAdding feature {}, improved R2 from {:.5f} to {:.5f}"
...
def rfe(data):
    """
    Perform recursive feature elimination to retrieve the 50 most relevant
    features based on feature importances

    Differs from 'tree_selection' only in the recursive nature (tree_selection
    only fits a model once, RFE fits a model
    50 times)

    :param data: a pandas dataframe where each row is an hour
    :return: a pandas dataframe with only the 50 most relevant features based
    on RFE
    """
    data = data.copy()
    ...
    print("\tPerforming recursive feature elimination based on a" +
          " Random Forest...")
